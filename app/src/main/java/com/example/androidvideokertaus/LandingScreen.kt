...
fun LandingScreen(
    ...
    LaunchedEffect(key1 = vm.isLoggedIn.value.done) {
        if(vm.isLoggedIn.value.done) {
            vm.setDone(false)
            goToLoginScreen()
        }
    }

    LaunchedEffect(key1 = vm.isLoggedIn.value.err) {
        vm.isLoggedIn.value.err?.let {
